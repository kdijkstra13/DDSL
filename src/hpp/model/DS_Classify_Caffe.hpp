...
namespace DSModel {
	...
	void Caffe<TClassType, TIdx, TId>::updateParameters() {
		...
		gpuDevices_ = this->template parameterValueById<Matrix<Int32>>("GPUDevices");
	...
	void Caffe<TClassType, TIdx, TId>::readModel(std::istream &input, const DSTypes::String &filename) {
		...
		string solverstatefile = SS(solver_->param().snapshot_prefix() << "_iter_" << currIter_ << ".solverstate");			
	...
	template<typename TClassType, typename TIdx, typename TId>
	Caffe<TClassType, TIdx, TId>::Caffe(const DSTypes::String &filename, const DSTypes::String snapshotModelFile) {
		init_();
	...
	Caffe<TClassType, TIdx, TId>::Caffe(const DSLib::Matrix<TClassType, TIdx> &classes, const DSTypes::String netProtoFile, const DSTypes::String solverProtoFile, const Matrix<Int32, TIdx> &gpuDevices, const TIdx maxIter, const Int32 seed) {
		...
		Table<TIdx, TId> tab =
			...
			(((Matrix<Literal, TIdx>() | "GPUDevices")			^ (ctParameter | (Matrix<Matrix<Int32>>() | gpuDevices))))
			);
		this->registerParameters(tab);
	}
	...
	void Caffe<TClassType, TIdx, TId>::clone(Caffe<TClassType, TIdx, TId> && other) {		
		clearCaffeModel();
		init_();
		gpuDevices_ = std::move(other.gpuDevices_);
		maxIter_ = other.maxIter_;
		currIter_ = other.currIter_;
		solverIter_ = other.solverIter_;
